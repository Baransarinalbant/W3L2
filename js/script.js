...
function getData(XMLcode) {
	let subjectElems = XMLcode.getElementsByTagName("subject"); // Lista (array) med alla subject-element
...
function selectCourses() {
	let course = this.value; // Ämne i menyns option-element
	requestInfo(course);
...
function requestInfo(value) {
	let request = new XMLHttpRequest(); // Object för Ajax-anropet
	request.open("GET","xml/" + value + ".xml",true);
	request.send(null); // Skicka begäran till servern
	request.onreadystatechange = function () { // Funktion för att avläsa status i kommunikationen
		if (request.readyState == 4) // readyState 4 --> kommunikationen är klar
...
function getInfo(XMLcode) {
	...
	let subjectElems = XMLcode.getElementsByTagName("course"); // Lista (array) med alla subject-element
	let HTMLcode = ""; // Textsträng med ny HTML-kod som skapas
